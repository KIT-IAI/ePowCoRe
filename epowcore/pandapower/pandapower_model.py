...
class PandapowerModel:
    ...
    def create_load_from_gdf(self, core_model: CoreModel, load: Load) -> bool:
        ...
        pandapower.create_load(
            ...
            const_z_p_percent=load.get_default(attr="const_z_percent"),
            const_i_p_percent=load.get_default(attr="const_i_percent"),
            const_z_q_percent=load.get_default(attr="const_z_percent"),
            const_i_q_percent=load.get_default(attr="const_i_percent"),
            sn_mva=np.nan,
            scaling=1.0,
            in_service=True,
    ...
    def create_line_from_gdf_tline(self, core_model: CoreModel, tline: TLine) -> bool:
        ...
        if False:  # tline.r1 * tline.length < 0.001 or tline.x1 * tline.length < 0.001:
            ...
            Logger.log_to_selected(
                f"Line {tline.name} was converted to a switch because of its short length of {tline.length} km."
            )
        else:
