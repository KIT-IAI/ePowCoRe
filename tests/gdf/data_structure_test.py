...
class DataStructureTest(unittest.TestCase):
    ...
    def test_corresponding_connector(self) -> None:
        ...
        ds.add_connection(buses[0], buses[1], "A1", "A2")
        ds.add_connection(buses[0], buses[2], "B1", "B2")
        ds.add_connection(buses[1], buses[2], "C1", "C2")
        self.assertEqual(ds.get_corresponding_connector(buses[0], buses[1], "A1"), "A2")
        self.assertEqual(ds.get_corresponding_connector(buses[1], buses[0], "A2"), "A1")
        self.assertEqual(ds.get_corresponding_connector(buses[1], buses[2], "C1"), "C2")
        self.assertEqual(ds.get_corresponding_connector(buses[2], buses[0], "B2"), "B1")
        self.assertIsNone(ds.get_corresponding_connector(buses[0], buses[2], "A1"))
        self.assertIsNone(ds.get_corresponding_connector(buses[2], buses[0], "A1"))

    # @unittest.skip("tmp")
    def test_subsystem(self) -> None:
        """Create subsystem from components, flatten it and check if the subsystem is removed from the data structure."""
        creator = GdfTestComponentCreator(100.0)
        buses = [creator.create_bus() for _ in range(4)]

        ds = creator.data_structure
        ds.add_connection(buses[0], buses[1], "A", "A")
        ds.add_connection(buses[1], buses[2], "B", "B")
        ds.add_connection(buses[2], buses[3], "C", "C")
        ds.add_connection(buses[3], buses[0], "D", "D")

        ds_copy = copy.deepcopy(ds)

        subsystem = Subsystem.from_components(ds, [buses[3]])

        self.assertEqual(len(ds.graph.nodes), 4)
        self.assertEqual(len(ds.graph.edges), 4)
        self.assertEqual(len(subsystem.graph.nodes), 3)
        self.assertEqual(len(subsystem.graph.edges), 2)

        attached = ds.get_attached_to(buses[0])
        ...
        self.assertEqual(ds.get_connector_names(buses[0]), ["A", "D"])
        self.assertEqual(ds.get_connection_name(buses[0], buses[1]), ["A"])

        flatten(ds)
        self.assertEqual(len(ds.graph.nodes), len(ds_copy.graph.nodes))
        self.assertEqual(len(ds.graph.edges), len(ds_copy.graph.edges))
        for l, r in itertools.combinations(ds.graph.nodes, 2):
            self.assertEqual(
                ds.get_connection_name(l, r),
                ds_copy.get_connection_name(l, r),
            )
            self.assertEqual(
                ds.get_connection_name(r, l),
                ds_copy.get_connection_name(r, l),
            )

    # @unittest.skip("tmp")
    def test_export_import(self) -> None:
        """Create data structure, export it, import it and check if the data structure is the same."""
        creator = GdfTestComponentCreator(100.0)
        buses = [creator.create_bus() for _ in range(3)]
        tline = creator.create_tline()
        shunt = creator.create_shunt()
        ds = creator.data_structure
        ds.add_connection(buses[0], shunt)
        subsystem = Subsystem.from_components(ds, [shunt])
        ds.add_connection(buses[1], buses[2], "A", "A")
        ds.add_connection(buses[2], tline, "B", "B")

        dict_export = ds.export_dict()
        data_str = json.dumps(dict_export, indent=2)
        if not os.path.isdir("./tests/out/gdf"):
            os.mkdir("./tests/out/gdf")
        with open("./tests/out/gdf/data_structure_gdf.json", "w", encoding="utf-8") as file:
            file.write(data_str)
        ds_import = DataStructure(base_frequency=50.0).import_dict(dict_export)
        self.assertEqual(len(ds.graph.nodes), len(ds_import.graph.nodes))
        ...
        for component in ds.graph.nodes:
    ...
    def test_get_attached_to(self):
        ...
        # print(ds.get_attached_to(buses[1]))
        # print(ds.get_attached_to(buses[2]))
        # print(ds.get_attached_to(buses[3]))
    ...
    def test_get_neighbors(self):
        ...
        subsystem_inner = Subsystem.from_components(ds, [buses[3]])
        subsystem_inner = Subsystem.from_components(ds, [subsystem_inner])
        subsystem_outer = Subsystem.from_components(ds, [subsystem_inner, buses[2]])
        ...
        # print(ds.get_neighbors(buses[2], follow_links=True))
